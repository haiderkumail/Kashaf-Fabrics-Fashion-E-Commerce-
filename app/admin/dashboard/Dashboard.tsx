...
import { useCurrency } from '@/components/header/CurrencyProvider';  // Import the currency context
    ...
    datasets: [
      {
        ...
        data: summary.salesData.map(
          (x: { totalSales: number }) => convertPrice(x.totalSales),  // Convert sales price
        ),
    ...
    datasets: [
      {
        ...
        label: 'Orders',
        data: summary.salesData.map(
          (x: { totalOrders: number }) => convertPrice(x.totalOrders),  // Convert orders price
        ),
        borderColor: 'rgb(53, 162, 235)',
        backgroundColor: 'rgba(53, 162, 235, 0.5)',
      },
    ...
    datasets: [
      {
        ...
        data: summary.productsData.map(
          (x: { totalProducts: number }) => convertPrice(x.totalProducts),  // Convert products count or price
        ),
        backgroundColor: [
          'rgba(255, 99, 132, 0.2)',
          'rgba(54, 162, 235, 0.2)',
    ...
    datasets: [
      {
        ...
        data: summary.usersData.map(
          (x: { totalUsers: number }) => convertPrice(x.totalUsers),  // Convert user data
        ),
      },
    ],
  };
      ...
      <div className='stats stats-vertical my-4 inline-grid shadow md:stats-horizontal md:flex'>
        <div className='stat'>
          ...
          <div className='stat-value text-primary'>
            {currency} {formatNumber(convertPrice(summary.ordersPrice))} {/* Display sales in selected currency */}
          </div>
